...
QString firstDirectiveCard(const QString& commandText) {
...
void SchematicEditor::createToolBar() {
    ...
    editMenu->addSeparator();
    editMenu->addAction(getThemeIcon(":/icons/tool_delete.svg"), "Delete", this, &SchematicEditor::onDelete, QKeySequence::Delete);
...
void SchematicEditor::onEditSimulationFromDirective(const QString& currentCommand) {
    auto applyDirectiveText = [this](SchematicSpiceDirectiveItem* item, const QString& newText) {
        if (!item) return;
    ...
    const QString card = firstDirectiveCard(currentCommand);

    if (card == ".tran" || card == ".ac" || card == ".op" || card == ".dc" || card.isEmpty()) {
